...
class VidaGSPOReward(ORM):
    
    def __call__(self, completions: List[str], target: List[str], **kwargs) -> List[float]:

        rewards = []
        encoder = get_reward_model()
        
        for text, label_str in zip(completions, target):
            total_score = 0.0
            try:
...
orms['vida_gspo_reward'] = VidaGSPOReward

print(">> [Plugin] Class 'VidaGSPOReward' registered to 'orms' successfully.")